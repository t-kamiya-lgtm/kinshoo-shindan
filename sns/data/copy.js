...
export const AXES = [
  {
    ...
    audience: 'trainee',
    audienceLabel: 'トレーニー向け',
    skuBias: 'monster'
  },
  {
    id: 'staple',
    label: '主食で摂る',
    desc: '主食そのものからたんぱく質を摂るという発想',
    audience: 'trainee',
    audienceLabel: 'トレーニー向け',
    skuBias: null
  },
  {
    id: 'meal',
    label: '主食をガマンしない',
    desc: '食事管理中でも麺を食べられるという食シーン',
    audience: 'dieter',
    audienceLabel: 'ダイエッター向け',
    skuBias: null
  },
  {
    id: 'daily',
...
export const HOOKS = {
  ...
  // ダイエッター向け。主食をガマンしなくていい、という一点で通す。
  meal: {
    change: [
      (p, n) => `主食を我慢しない、という選択肢。`,
      (p, n) => `プロテインの麺だから、主食をガマンしなくていい。`,
...
export const BODIES = {
  ...
  staple: {
    change: (p, n, c) => [
      `${p.nameJa}は${p.category}。主食でプロテインが摂れます。`,
  ...
  meal: {
    change: (p, n, c) => [
      `1食（${p.servingG}g）あたり ${n.kcal}kcal／たんぱく質${n.protein}g／脂質${n.fat}g／糖質${n.sugar}g／食物繊維${n.fiber}g。`,
      `${p.nameJa}は${p.category}。まさにプロテイン麺です。`,
      `ゆで時間は${p.boilMin}分、ゆであがりは約${p.cookedWeightG}g。食べごたえのある量です。`,
      `我慢して主食を減らすのではなく、食べるものを入れ替える。そういう考え方の麺です。`
    ],
    add: (p, n, c) => [
 ...
 * ------------------------------------------------------------------ */
/* 締めは読み手ごとに持つ。
   共通プールにすると、ダイエッター向けの本文に「飲むより、食べる。」のような
   トレーニー向けの締めが混ざり、1投稿の中で相手が入れ替わってしまう。 */
...
export const CLOSINGS_X = {
  trainee: [`詳細はプロフィールのリンクから。`, `スペックの詳細はプロフィールのリンクへ。`],
  dieter: [`主食をガマンしない毎日を、プロフィールのリンクから。`, `詳細はプロフィールのリンクから。`],
  both: [`気になった方はプロフィールのリンクをどうぞ。`, `詳細はプロフィールのリンクから。`]
};
/* ------------------------------------------------------------------ *
 * 注意喚起（アレルギー）。全投稿の末尾に必ず入れる。
 * ------------------------------------------------------------------ */
/**
 * アレルギー表示。商品ごとに内容が違うので、必ず products.js から引く。
 * モンスターの製造工場でそばは扱っていない。そばを含むのは SOVA の原材料。
 */
export const allergyNotes = (p) => (p.allergyNotes || []).join('\n');
